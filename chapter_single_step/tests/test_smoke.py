...
class DatasetTests(unittest.TestCase):
    def test_dataset_counts(self):
        ...
        self.assertEqual(dataset["template_instances"], 1377)
        self.assertEqual(dataset["unique_templates"], 1005)
        self.assertEqual(len(dataset["templates"]), 1005)
        original_reactions = json.loads(
            (PACKAGE_DIR.parent / "reaction_template.json").read_text(encoding="utf-8")
        )
        self.assertEqual(len(original_reactions), 1377)

        preprocessed_path = PACKAGE_DIR.parent / "preprocessed_data.csv"
        preprocessed_text = preprocessed_path.read_text(encoding="utf-8-sig")
        self.assertNotIn("\u00a1\u00e3C", preprocessed_text)
        with preprocessed_path.open(encoding="utf-8-sig", newline="") as handle:
            preprocessed = list(csv.DictReader(handle))
        executable_templates = [
            row["template_r1"]
            for row in preprocessed
            if row["template_r1"]
            and ">>" in row["template_r1"]
            and "." not in row["template_r1"].split(">>", 1)[0]
        ]
        self.assertEqual(executable_templates, original_reactions)

        condition_path = PACKAGE_DIR.parent / "template_condition.json"
        condition_text = condition_path.read_text(encoding="utf-8")
        self.assertNotIn("\u00a1\u00e3C", condition_text)
        conditions = json.loads(condition_text)
        self.assertEqual(set(conditions), set(original_reactions))
        self.assertEqual(
            sum(row["instance_count"] for row in dataset["templates"]),
            len(original_reactions),
        )
        self.assertEqual(len({row["template_hash"] for row in dataset["templates"]}), 1005)
        self.assertEqual(sum(dataset["new_unique_templates_by_chapter"].values()), 1005)
        ...
        self.assertEqual(sum(int(row["templates"]) for row in summary), 1377)
        self.assertEqual(sum(int(row["unique_templates"]) for row in summary), 1005)
    ...
    def test_known_chapters_5_6_7_prediction(self):
        ...
        self.assertEqual(result["template_inventory"]["available_unique_templates"], 400)
        self.assertEqual(
    ...
    def test_chapter_5_dihydroxylation_templates(self):
        ...
        self.assertEqual(result["template_inventory"]["available_unique_templates"], 320)
    ...
    def test_chapters_without_templates_are_accepted(self):
        ...
        self.assertEqual(result["template_inventory"]["available_unique_templates"], 25)
        self.assertEqual(
            result["template_inventory"]["unique_templates_by_requested_chapter"],
            {"1": 0, "2": 25, "3": 0},
        )
